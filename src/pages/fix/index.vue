...
const fixObj = reactive({
  ...
  reload: () => {
    queryPreregister({
      page: 1,
      pageSize: 999999,
      startDate: fixObj.search.startTime,
      endDate: fixObj.search.endTime,
      queryCompany: "",
      queryName: "",
      queryPreregisterNo: "",
      querySerialNo: "",
      type: fixObj.search.active
    }).then(res => {
      if (res.code == 0) {
        fixObj.list = res.data.list.reverse()
      }
    })
  }
})
    ...
    <van-tab title="全部" class="m-[10px] w-[100vw] flex flex-col gap-[10px]">
      <div v-for="(item, index) in fixObj.list"
        ...
        <div name="顶栏" class="w-[100%] flex justify-between">
          ...
          <div name="状态">
            <h1 class="leading-none m-0 text-[12px] font-bold color-[#E7845B]">未维修</h1>
          </div>
        </div>
        ...
        <p class=" bg-[#FAFAFA] p-5px w-100% ">
          <a class="text-12px font-bold">反馈状态:</a>
          <a class="text-12px">设备没有修好</a>
        </p>
        <div class="flex flex-row justify-end w-100% gap-10">
          <VanButton class="w-[90px]" round size="small" @click="fixObj.model.feedback.show = true">详细</VanButton>
          <van-popup id="fu" class="h-50vh bg-[#f2f4f8] flex flex-col p-10 items-center rounded-t-[10px]" v-model:show="fixObj.model.feedback.show" position="bottom">
              ...
              <van-field v-model="fixObj.model.feedback.message" class="rounded-[10px]" rows="2" autosize
          ...
          <VanButton class="w-[90px]" color="#E7845B" round size="small" @click="to('fixnote')">结果反馈</VanButton>
        </div>
      </div>
    </van-tab>
