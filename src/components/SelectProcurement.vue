...
const procurement = defineModel("procurement", {
    default: {}
})
const selectPurchase = reactive({
    name: "",
    list: [],
    reload: () => {
        //TODO:无限屏
        queryPurchase({
            ...
            "searchKey": "",
            "searchProductNo": "",
            "startTime": "1993-04-11",
            "endTime": "2099-04-22",
            "searchType": "",
            "searchStatus": 0
        }).then(res => {
            if (res.code == 0) {
                selectPurchase.list = res.data.list.reverse()
            }
        })
    }
})
